process ISOFORM_SWITCH_PART1_LONG {
    container 'https://depot.galaxyproject.org/singularity/bioconductor-isoformswitchanalyzer:2.6.0--r44h3df3fcb_0'
    publishDir "${params.output_dir}/long_read/06_biological_analysis/isoform_switches/part1", mode: 'copy'
    ...
    cat("=== Initialising Part 1 (Long Read) ===\\n")
    ...
    switchList <- isoformSwitchTestDEXSeq(
        switchAnalyzeRlist = switchList,
        reduceToSwitchingGenes = FALSE, # Prevent crash if 0 significant switches found
        dIFcutoff = 0.1
    )
    ...
    try(print(extractSwitchSummary(switchList, dIFcutoff = 0.1)))
    sink()
    """
}
