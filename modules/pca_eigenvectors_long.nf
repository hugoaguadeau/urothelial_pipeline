// Process: PCA Eigenvectors Analysis (Long Reads)
process PCA_EIGENVECTORS_LONG {
    ...
    publishDir "${params.output_dir}/long_read/05_differential_expression/pca_analysis", mode: 'copy'
    ...
    # ========================================================================
    # PCA EIGENVECTORS ANALYSIS - LONG READS
    # ========================================================================
    ...
    sample_info <- sample_info_full[sample_info_full\$seq_type == "long", ]
    ...
    # Get raw tissue info
    raw_tissue <- tissue_map[rownames(pca_data)]
    
    # Rename labels: 'bladder' -> 'Bladder', 'ureter' -> 'Upper Tract'
    clean_tissue <- ifelse(tolower(raw_tissue) == "ureter", "Upper Tract",
                    ifelse(tolower(raw_tissue) == "bladder", "Bladder", raw_tissue))
    
    pca_data\$Tissue <- factor(clean_tissue, levels = c("Bladder", "Upper Tract"))
    ...
    p_basic <- ggplot(pca_data, aes(x = PC1, y = PC2, colour = Tissue)) +
        geom_point(size = 8) +
        scale_colour_manual(values = c("Bladder" = "#E41A1C", "Upper Tract" = "#377EB8")) +
        labs(
    ...
    save_both_formats(p_basic, "pca_by_tissue")
    ...
    # Plot 3: RIN PCA (Updated to fix overlapping numbers)
    if(has_rin) {
        p_rin <- ggplot(pca_data, aes(x = PC1, y = PC2, colour = RIN)) +
            ...
            # This 'guides' line fixes the overlapping numbers by making the bar much wider
            guides(colour = guide_colourbar(barwidth = 25, barheight = 1.5, title.position = "top")) +
            labs(
    ...
    save_both_formats(p_biplot, "pca_biplot", width = 14, height = 12)
