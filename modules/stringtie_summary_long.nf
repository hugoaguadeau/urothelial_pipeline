// Process: StringTie Summary - Analyse transcript quantification results (Long Reads)
process STRINGTIE_SUMMARY_LONG {
    container 'https://depot.galaxyproject.org/singularity/r-base:4.3.1'
    publishDir "${params.output_dir}/long_read/04_quantification/summaries", mode: 'copy'
    ...
    # ============================================================================
    # StringTie Quantification Summary (Long Read RNA-seq)
    # ============================================================================
    ...
    for (sample_id in sample_ids) {
        ...
        # Note: Long reads typically have lower coverage, so thresholds are adjusted
        genes_low <- sum(gene_sample > 0 & gene_sample <= 5)
        genes_medium <- sum(gene_sample > 5 & gene_sample <= 50)
        genes_high <- sum(gene_sample > 50)
        
        # Transcript-level statistics
        transcript_sample <- transcript_counts[[sample_id]]
        transcripts_detected <- sum(transcript_sample > 0)
        
        # Calculate transcripts per gene
    ...
    cat("================================================================================\\n")
    cat("              STRINGTIE QUANTIFICATION SUMMARY (LONG READS)\\n")
    cat("================================================================================\\n\\n")
